...
# ╔═╡ 75e8b966-79ce-496a-8121-bec9d4ebfbc4
md"""
...
# @parameters missing
@parameters R=200 Qin=0.01 Tin=30.0 Uc=4000 Tc=15.0 ρ=1000 cₚ=4200 A=0.283 d=0.6
...
# change_heat = missing
change_heat = D(H) ~ ρ*cₚ*Qin*Tin - ρ*cₚ*sqrt(h)/R*T - Uc*(A + π*d*h)*(T - Tc)

# ╔═╡ 70e23f70-aac4-46e6-b20c-18947e0153d7
# change_volume = missing
change_volume = D(V) ~ Qin - sqrt(h)/R
...
# eqns_tank = missing
eqns_tank = [eq_heat, eq_volume, change_heat, change_volume]
...
# @mtkbuild missing
@mtkbuild sys1_tank = ODESystem(eqns_tank, t)
...
# oprob1_tank = missing
oprob1_tank = ODEProblem(sys1_tank, [T=>15.0, h=>0.8], (0.0, 2400.0), [Qin=>0.01])
...
# sol1_tank = missing
sol1_tank = solve(oprob1_tank, saveat=1, reltol=1e-9)
...
# end
begin
	plot(sol1_tank; idxs=[T], ylabel="Temperature [°C]", legend=:bottomleft)
	plot!(twinx(), sol1_tank; idxs=[h],color=:red, ylabel="Height [m]", xlabel="", legend=:bottomright)
end
...
# @mtkbuild missing
@mtkbuild sys2_tank = ODESystem(eqns_tank, t; discrete_events=[[1200]=>[R~250]])
...
# oprob2_tank = missing
oprob2_tank = ODEProblem(sys2_tank, [T=>15.0, h=>0.8], (0.0, 2400.0), [])
...
# sol2_tank = missing
sol2_tank = solve(deepcopy(oprob2_tank), saveat=1, reltol=1e-9)
...
# end
begin
	plot(sol2_tank; idxs=[T], ylabel="Temperature [°C]", legend=:bottomleft)
	plot!(twinx(), sol2_tank; idxs=[h],color=:red, ylabel="Height [m]", xlabel="", legend=:bottomright)
end
