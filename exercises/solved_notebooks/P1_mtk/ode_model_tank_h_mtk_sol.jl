...
# @parameters missing
@parameters A=0.152 ρ=1000.0 g=9.81 Qin R
...
change_height = A*D(h) ~ Qin - (h>0)*sqrt(h)/R
# (h > 0)   # to make sure that h does not become negative...
# >(h, 0)
...
# eqns_tank = missing
eqns_tank = [change_height, eq_pressure]
...
# oprob1_tank = missing
oprob1_tank = ODEProblem(sys1_tank, [h=>h₀], (0.0, 1800.0), [Qin=>QinLmin*1e-3/60, R=>R_val])

# ╔═╡ 44878548-9661-421e-a9ab-b82ca92a6310
# sol1_tank = missing
sol1_tank = solve(oprob1_tank, Tsit5(), saveat=1)
...
# @mtkbuild missing
@mtkbuild sys2_tank = ODESystem(eqns_tank, t; continuous_events = [[h ~ 0.99]=>[Qin ~ 0]])
...
# oprob2_tank = missing
oprob2_tank = ODEProblem(sys2_tank, [h=>0.0], (0.0, 1800.0), [Qin=>100*1e-3/60, R=>600])
...
# sol2_tank = missing
sol2_tank = solve(deepcopy(oprob2_tank), Tsit5(), saveat=1)
...
# i_max = missing
i_max = findfirst(==(0.99), sol2_tank[h])

# ╔═╡ 1c21b251-7ef4-4f07-a7f1-cf434fdaaac3
# Δi_zero = missing
Δi_zero = findfirst(<(0), sol2_tank[h][i_max:end])
...
# oprob3_tank = missing
oprob3_tank = ODEProblem(sys3_tank, [h=>0.0], (0.0, 1800.0), [Qin=>100*1e-3/60, R=>600])
...
# sol3_tank = missing
sol3_tank = solve(deepcopy(oprob3_tank), Tsit5(), saveat=1)
...
# missing
plot(sol3_tank; idxs=[h, p], ylim=(0, 1.25))
