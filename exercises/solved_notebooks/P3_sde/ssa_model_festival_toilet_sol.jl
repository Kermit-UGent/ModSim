...
# dprob = missing;
dprob = DiscreteProblem(toilet, [], tspan);

# ╔═╡ b9bddaa3-4293-40a0-8b5b-a277d4fb9f5f
# jprob = missing;
jprob = JumpProblem(toilet, dprob);
# jprob = JumpProblem(toilet, dprob, Direct());
...
begin
	...
	for i = 1:1000
		sol2 = solve(jprob)
		append!(line_maxima, maximum(sol2[:L]))
	end
end
...
# rush_event = [missing, missing]
rush_event = [[120] => [toilet.kₐ ~ 3.0], [180] =>[toilet.kₐ ~ 1.5]]

# ╔═╡ 9d6ead94-0ac4-4a22-b091-551463151328
# @named toilet_with_rush = missing;
@named toilet_with_rush = ReactionSystem(equations(toilet), discrete_events=rush_event);

# ╔═╡ aa63d527-f671-4de2-beb6-16736579e18a
# dprob3 = missing;
dprob3 = DiscreteProblem(complete(toilet_with_rush), [], tspan, discrete_events=rush_event);

# ╔═╡ b72661ec-3b48-481f-94a2-bdcad402323c
# jprob3 = missing;
jprob3 = JumpProblem(complete(toilet_with_rush), dprob3);
# jprob3 = JumpProblem(complete(toilet_with_rush), dprob3, Direct());
