...
\frame{
	...
	\begin{itemize}
		...
		\item Exceptions: see \textbf{Course timeline} or \textbf{agenda} in Ufora
		\vfill %\pause
...
\frame{
\frametitle{Set up your environment}
\begin{enumerate}
\item[1.] Go to Ufora to the module \textbf{Exercises} $>$ \textbf{Important files} and download the zip file \texttt{modsim.zip}\medskip
\item[2.] Unzip this file (in your \texttt{Downloads} folder). \medskip
\item[3.] Move the folder \texttt{modsim\_practica} to a suitable location (e.g. \texttt{Documents}) \medskip
%\item[1.] Create a folder (e.g., \texttt{modsim}) in a suitable location (e.g., \texttt{Documents})
\begin{itemize} %\medskip
\item In principle this can be any location, choose an easily accessible one \smallskip
\item Preferably don't leave it in the \texttt{Downloads} folder...
\end{itemize} \medskip
\item[4.] Go to your \texttt{modsim\_practica} folder with a \textit{File Explorer}, then open in this location the \textit{Terminal} or command line interface (CLI), e.g., right click on your mouse and choose \texttt{Open in Terminal} or type \texttt{cmd} in the address bar. \medskip
\item[5.] Type {\color{ugent-la} \texttt{julia launch\_pluto.jl}} and then click [\textit{Enter}] \medskip
\begin{itemize}
\item Packages will be installed, this might take a few minutes...
\end{itemize}

%\item[3.] Now create a new folder \texttt{P1\_ODEs1} (in \texttt{modsim}) \medskip
%\item[4.] Go to Ufora to the module \textbf{Exercises} $>$ \textbf{Important files} and download
%\begin{itemize}
%\item \texttt{Project.toml}
%\item \texttt{launch\_pluto.jl}
%\end{itemize}
%Put these two files in your \texttt{modsim} folder.
\end{enumerate}
}
...
\frametitle{Set up your environment}
\begin{itemize} 
\item The script {\color{ugent-la} \texttt{launch\_pluto.jl}} serves to activate your environment and to be able to use the packages that are installed into this environment (cf. \texttt{modsim\_practica}). \medskip
\item It is possible to run the notebooks without pre-installed packages BUT then your notebook will need to upload the necessary packages on site into a temporary notebook environment each time you run a notebook and THIS TAKES A LOT OF TIME! \medskip
\item {\color{ugent-la} \texttt{Project.toml}} is a file with the names of the packages and their versions (coded!) that we will use. \medskip
\item Running {\color{ugent-la} \texttt{launch\_pluto.jl}} with {\color{ugent-la} \texttt{Project.toml}} in the same folder installs the right packages automatically.
\begin{itemize}
\item This allows that everyone work with the same packages versions.
\item This avoids problems with package versions.
\end{itemize}
\end{itemize}
}
...
\frame{
\frametitle{Download the notebooks}
\begin{enumerate}
\item[6.] Go to Ufora to the module \textbf{Exercises} $>$ \textbf{P1: ODEs 1} $>$ \textbf{files} and download the the following notebooks \medskip
\begin{itemize}
\item \texttt{ode\_model\_catalyst\_intro.jl}
\item \texttt{ode\_model\_infection.jl}
\item \texttt{ode\_model\_birth\_death.jl}
\item \texttt{ode\_model\_irrigation.jl}
\end{itemize}
You can download the rest later\medskip
\item[7.] Move all these notebooks to your subfolder \texttt{P1\_ODE1}\medskip
\item[8.] When the installation is complete, you will be able to open the notebooks in the folder \texttt{P1\_ODE1}
\end{enumerate}
}
...
%\frametitle{Set up your environment}
%\begin{enumerate}
%\item[5.] Go to Ufora to the module \textbf{Exercises} $>$ \textbf{P1: ODEs 1} and download all the Pluto notebook files
%\begin{itemize}
%\item \texttt{ode\_model\_catalyst\_intro.jl}
%\item \texttt{ode\_model\_infection.jl}
%\item etc.
%\end{itemize}
%Put all these files in the folder \texttt{P1\_ODEs1}.\medskip
%\item[6.] Go back your \texttt{modsim} folder with a \textit{File Explorer}, then open in this location the \textit{Terminal} or command line interface (CLI), e.g., right click on your mouse and choose \texttt{Open in Terminal}. \medskip
%\item[7.] Type {\color{ugent-la} \texttt{julia launch\_pluto.jl}} and then click [\textit{Enter}]
%\begin{itemize}
%\item Packages will be installed, this might take a few minutes...
%\item When installation in complete, you will be able to open the notebooks in the folder \texttt{P1\_ODEs1}
%\end{itemize}
%\end{enumerate}
%}
...
\begin{figure}
	\includegraphics[width=0.6\textwidth]{figs/infection_model_catalyst_implementation.png}
\end{figure}

$S$, $I$, $D$ and $R$ $\rightarrow$ species, unknowns, states or variables\\
...
\begin{itemize}
\item runoff: $r \cfrac{S_1 + S_{1,res}}{S_{max}}$
\item percolation: $\cfrac{k}{S_{max}} S_1$
\item outflow: $v\,S_2^2$
\end{itemize}
...
\begin{figure}
	\includegraphics[width=0.6\textwidth]{figs/anaerobic_fermentation.png}
\end{figure}
