...
# ╔═╡ 3d847ad8-fee3-4fe9-b8d3-eba45f83eb22
...
# @parameters missing
@parameters m=23.0 g=9.81 k=22570 L=0.40 b=900
...
let
	...
	round(L - m*g/k, digits=2)
	# 0 = -m*g + k*(L - y)
	# <=> -k*y = m*g - k*L    # is of the form Ax=b => x = A\b
	round((-k)\(m*g-k*L), digits=2)
end
...
# eq_position = missing
eq_position = m*D(D(y)) ~ - m*g + k*(L - y) - b*D(y)
...
# @mtkbuild missing
@mtkbuild sys_tractor_seat = ODESystem(eq_position, t; discrete_events = [[1]=>[m ~ m+M]])
...
oprob_treactor_seat = ODEProblem(sys_tractor_seat, [y=>0.39, D(y)=>0.0], (0.0, 5.0), [b=>b_val])

# ╔═╡ 7c1edab7-722d-4014-90c0-1b834e80aec7
# sol_tractor_seat = missing
sol_tractor_seat = solve(deepcopy(oprob_treactor_seat), Tsit5(), saveat=0.01);
...
# missing
plot(sol_tractor_seat; idxs=[y], ylim=(0.32, 0.392))
...
# missing
plot(sol_tractor_seat; idxs=[D(y)])
...
# stst_val = missing
stst_val = solve(SteadyStateProblem(sys_tractor_seat, [y=>0.35, D(y)=> 0.0], [m=>23.0+M]))
