...
# ╔═╡ 41ac2762-edb9-11ee-1470-6f9ec93c0985
using Turing, Plots

# ╔═╡ 5dfb5d11-2b0b-48c0-9362-e8497a124144
md" # Gravitation
Two cubic blocks, both with width, length, and height of 1 meter, are placed next to each other. They both have a density of 1 kg/m$^2$. Newton's law of universal gravitation gives the gravitational force between two point masses of a distance $d$ between another:

$$F = G \frac{m_1m_2}{d^2}\,,$$

with $G$ the gravitational constant of 6.674$\times10^{−11}$ N m$^2$/kg$^2$. Use samping to estimate the nett force between the blocks. You can use $G=1$ for convenience.
"

# ╔═╡ 995ff1a5-5f74-4da8-82f4-d1345e92b958
md"method 1: all at once"

# ╔═╡ bfc053a1-8ddc-4c12-8b5e-75185debeee5
@model function gravity(rho1=1, rho2=1)
	G = 1 # gravitational constant is equal to 1 for simplicity
	# sample a point from the first block
	x1 ~ Uniform(0, 1)
	y1 ~ Uniform(0, 1)
	z1 ~ Uniform(0, 1)
	# sample a point from the second block
	x2 ~ Uniform(1, 2)
	y2 ~ Uniform(0, 1)
	z2 ~ Uniform(0, 1)
	# compute distance
	r_squared = (x1 - x2)^2 + (y1 - y2)^2 + (z1 - z2)^2
	# compute force and save in a Dirac distribution
	F ~ Dirac(G * rho1 * rho2 / r_squared)
end

# ╔═╡ 68ca683a-87e1-44a0-8778-ce1f60a1bb5f
mean([rand(gravity())[:F] for i in 1:10_000])

# ╔═╡ a7363ba3-5392-490d-9fa5-f02596f77230
md"method 2: in two blocks"

# ╔═╡ 34461ad0-aa22-4d2a-b82c-7489fb5ad27d
@model function cube(x0, y0, z0)
	G = 1 # gravitational constant is equal to 1 for simplicity
	# sample a point from a block with a certain position
	x ~ Uniform(x0, x0+1)
	y ~ Uniform(y0, y0+1)
	z ~ Uniform(z0, z0+1)
end

# ╔═╡ 9a177716-4f3d-487e-94d1-87b295c0e84e
rand(cube(0, 0, 0))  # random point from a cube of with bottom vertex at (0, 0, 0)

# ╔═╡ 288e7239-6dbb-432d-adee-fc15d805bddc
rand(cube(1, 0, 0))  # random point from a cube of with bottom vertex at (1, 0, 0)

# ╔═╡ 248d21cf-e2ca-4a97-b9fe-f49d83d81651
force((x1, y1, z1), (x2, y2, z2)) = 1 / ((x1 - x2)^2 + (y1 - y2)^2 + (z1 - z2)^2)

# ╔═╡ 1e95ac53-c27f-4906-bc64-75148e4c7d15
forces = [force(rand(cube(0, 0, 0)), rand(cube(1, 0, 0))) for i in 1:10_000]

# ╔═╡ c59b4393-50d8-4b3a-81c5-32b12e699cff
mean(forces)
