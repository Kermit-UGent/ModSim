...
parms1 = [:α => 0.08, :β => 1.0e-6, :r => r, :m => 0.4] 	# r specified by slider

# ╔═╡ d56b486e-44e5-4db7-9c4e-00db5c1b733f
oprob1 = ODEProblem(infection_model, u0, tspan, parms1);
...
oprob2 = ODEProblem(infection_model2_com, u0, tspan, parms)
...
oprob3 = ODEProblem(infection_model3_c_com, u0, tspan, parms)
...
md"
Initialize vector `parms_ex1` with parameter values:
"
...
parms_ex1 = [:α => α, :β => 1.0e-6, :r => 0.2, :m => 0.4]

# ╔═╡ 7955722d-fc35-4cce-8bf2-ee40ee7fbc82
oprob_ex1 = ODEProblem(infection_model, u0, tspan, parms_ex1);
...
parms_ex2 = [:α => 0.08, :β => 1.0e-6, :b => b, :m => 0.4, :r => 0.2, :h => 0.5]

# ╔═╡ ec08adb2-561c-4f17-a0b5-019d7b1f1098
oprob_ex2 = ODEProblem(infection_med, u0, tspan, parms_ex2);
...
parms_ex3 = [:α => 0.08, :β => 1.0e-6, :b => 0.2, :m => 0.4, :r => 0.2, :h => 0.5, :v => v]

# ╔═╡ c61ae8b6-2324-43fa-a5ae-274a919af559
oprob_ex3 = ODEProblem(infection_med_vac, u0, tspan, parms_ex3);
...
oprob_ex3_c = ODEProblem(infection_med_vac_c_com, u0, tspan, parms_ex3);
...
md"""
!!! question
	What can you say about the rate of infection in the case of no vaccination? How would you measure this in the plot?
"""
