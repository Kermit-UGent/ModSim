...
# ╔═╡ aa1f14d4-f102-11ee-08b0-75fa614b0bec
using Turing, Plots

# ╔═╡ 88d0f915-8ca1-48ab-a69b-4d9d7b8f7f47
@model function piecewise_poisson(x, y)
	gamma ~ InverseGamma(1) #~ InverseGamma(.01)  # scale for parameters
	# slope and intercept first line
	a1 ~ TruncatedNormal(0., gamma, 0, 100)
	# slope and intercept second line
	a2 ~ TruncatedNormal(0., gamma, 0, 100)
	# change point
	xcrit  ~ Uniform(minimum(x), maximum(x))
	for i in 1:length(x)
		if x[i] < xcrit
			y[i] ~ Poisson(a1)
		else
...
# ╔═╡ a2ca7ccc-5590-4afe-bf6c-7470d7fd2f2d
@model function piecewise_regression(x, y)
	alpha ~ InverseGamma(.1)
	sigma ~ InverseGamma(alpha)  # variance measurement error
	gamma ~ InverseGamma(alpha)  # scale for parameters
	# slope and intercept first line
	a1 ~ Normal(0., gamma)
	b1 ~ Normal(0., gamma)
	# slope and intercept second line
	...
	for i in 1:length(x)
		if x[i] < xcrit
			y[i] ~ Normal(a1 * x[i] + b1, sigma)
		else
			y[i] ~ Normal(a2 * x[i] + b2, sigma)
		end
	end
end

# ╔═╡ 8b34525a-ceca-442b-b750-403352faeeb7
year = [1982, 1983, 1984, 1985, 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022] .- 2000

# ╔═╡ 8588decf-eb20-4747-ac04-6666f00223ee
# https://www.statista.com/statistics/811504/mass-shooting-victims-in-the-united-states-by-fatalities-and-injuries/

# ╔═╡ 068110b9-f80e-4c9d-b760-d0f33d32b6b3
fatalities = [8, 0, 28, 0, 15, 6, 7, 15, 10, 35, 9, 23, 5, 6, 6, 9, 14, 42, 7, 5, 0, 7, 5, 17, 21, 53, 17, 39, 9, 19, 71, 35, 18, 46, 71, 117, 80, 73, 9, 43, 74]

# ╔═╡ 5ed3669a-c7a4-4923-b7e6-9e6ecdd155d7
cumulative_fatalities = cumsum(fatalities)

# ╔═╡ 692020f9-038d-45f9-8351-2bf6f9976481
scatter(year, fatalities)

# ╔═╡ 319b44d1-e6ef-41bb-ac50-f3e41f491777
scatter(year, cumulative_fatalities)

# ╔═╡ 7f7fffbe-39a9-45ec-90f8-41dcbe188824
700/20

# ╔═╡ 039961a6-9925-4889-8381-fc3153a61c7e
chain1 = sample(piecewise_poisson(year, fatalities), NUTS(), 1_000);

# ╔═╡ a9880d73-8b18-4cba-aabe-c5b5a216daf5
summarize(chain1)

# ╔═╡ f95725f6-f45e-4b44-8101-bbcd8f1e2eca
chain2 = sample(piecewise_regression(year, cumulative_fatalities), NUTS(), 1_000);

# ╔═╡ 08fd6162-4961-423e-a81d-ee483b32dfcf
summarize(chain2)

# ╔═╡ 9e3073c5-0a47-43bb-9354-7fca3f90710b
post_means = mean(chain2)

# ╔═╡ 4d1194e0-a5d0-4d2c-91a6-c9809caacd8b
a1_est = mean(chain2[:a1])

# ╔═╡ 518c07e6-f39c-4029-9d34-1bc81ad36378
b1_est = mean(chain2[:b1])

# ╔═╡ 01fa097c-d7de-46f8-96c1-f444e05ce641
a2_est = mean(chain2[:a2])

# ╔═╡ 4eff0b12-30b3-4285-9e4b-d807a3f1a3db
b2_est = mean(chain2[:b2])

# ╔═╡ c7c55949-002e-4c90-9795-41fac6b91a0c
crit_est = mean(chain2[:xcrit])

# ╔═╡ 74944174-653d-4ce4-98d8-479de2c639fd
begin
	scatter(year, cumulative_fatalities, label="cumulative fatalities", ylims=extrema(cumulative_fatalities), xlab="year - 2000")
	plot!(year, year .* a1_est .+ b1_est, label="trend before xcrit")
	plot!(year, year .* a2_est .+ b2_est, label="trend after xcrit")
	vline!([crit_est], label="xcrit")
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "4a734bf30e71c34153ca571d46e03d07b64177bf"
