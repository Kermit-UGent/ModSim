...
md"
The following system of differential equations models the decay of a pollutant in soil and its uptake by plants. The variable $C(t)$ be the concentration of the pollutant in the soil and $P(t)$ be the concentration of the pollutant in the plants at time $t$.

$$\begin{align}
\cfrac{dC}{dt} &= C_{in}-k_1 C(t)-k_2 C(t) P(t)\\
\cfrac{dP}{dt} &= k_2 C(t) P(t)-k_3 P(t)
\end{align}$$

The interpratation of the parameters is the following:

-  $C_{in}$ represents the rate at which the pollutant enters the soil from external sources.
...
md"
Suppose that we start an experiment in which we simulate the evoluation of the popllutant in the soil and plant during $400$ days. The inital pollutant concentrations in the soil and plant both have the value of $0.001\;mg/kg$. In the experiment, the soil is being contaminated at a rate $0.06\;mg/(kg \cdot day)$. The degradation rates and uptake coefficient have the following values: $k_1 = 4.1 \times 10^{-3}\;day^{-1}$, $k_2 = 1.9 \times 10^{-2}\;kg/(mg \cdot day)$, $k_3 = 2.2 \times 10^{-2}\;day^{-1}$.
"
...
md"
Solve the ODE problem and store the solution in `osol`:
"
...
md"
Interprate the simulation results in terms of the used parameter values.
"
