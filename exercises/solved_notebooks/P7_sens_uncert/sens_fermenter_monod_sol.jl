...
parms = [:μmax => μmax_val, :Ks => Ks_val, :Sin => Sin_val]
...
# oprob = missing
oprob = ODEProblem(fermenter_monod, [], tspan, parms, combinatoric_ratelaws=false)

# ╔═╡ 89a31c32-88a4-479f-a688-ffcb75ee8e91
# osol = missing
osol = solve(oprob, Tsit5(), saveat=dt)
...
# end
function fermenter_monod_sim(parms)
	μmax_val, Ks_val, Sin_val = parms
	parms_new = [:μmax => μmax_val, :Ks => Ks_val, :Sin => Sin_val]
	oprob = ODEProblem(fermenter_monod, [], tspan, parms_new, combinatoric_ratelaws=false)
	osol = solve(oprob, Tsit5(), saveat=dt)
	return osol
end
...
fermenter_monod_sim_S(parms) = fermenter_monod_sim(parms)[:S]

# ╔═╡ 3ae5bd00-2e06-4789-aab3-d897824d5e29
# fermenter_monod_sim_X(params) = missing
fermenter_monod_sim_X(parms) = fermenter_monod_sim(parms)[:X]
...
# S_sim = missing
S_sim = fermenter_monod_sim_S([μmax_val, Ks_val, Sin_val])

# ╔═╡ 95bc683c-f6e6-4b42-b90b-b5a863edd4d5
# X_sim = missing
X_sim = fermenter_monod_sim_X([μmax_val, Ks_val, Sin_val])
...
# sens_S = missing
sens_S = ForwardDiff.jacobian(fermenter_monod_sim_S, [μmax_val, Ks_val, Sin_val])

# ╔═╡ 49a94b9a-a543-495e-b4f1-c8579e59304d
# sens_X = missing
sens_X = ForwardDiff.jacobian(fermenter_monod_sim_X, [μmax_val, Ks_val, Sin_val])
...
# end
begin
	sens_S_on_μmax_rel = sens_S_on_μmax .* μmax_val ./ S_sim
	sens_S_on_Ks_rel   = sens_S_on_Ks .* Ks_val ./ S_sim
	sens_S_on_Sin_rel  = sens_S_on_Sin .* Sin_val ./ S_sim
end;
...
# end
begin
	sens_X_on_μmax_rel = sens_X_on_μmax .* μmax_val ./ X_sim
	sens_X_on_Ks_rel   = sens_X_on_Ks .* Ks_val ./ X_sim
	sens_X_on_Sin_rel  = sens_X_on_Sin .* Sin_val ./ X_sim
end;
...
# missing
plot(t_vals, [sens_S_on_Sin_rel, sens_X_on_Sin_rel], title="Normalized sensitivities", label=["S w.r.t. Sin" "X w.r.t. Sin"], xlabel="Time (hours)", linewidth=2)
...
"""
#=
- X is most sensitive to Sin because the higher Sin, the more X can be produced.
- In the beginning, the substrate S is positively affected by Sin because S enters the tank through Sin, and only a little biomass X is present, so the biomass cannot consume the substrate very fast.
=#
...
# missing
plot(t_vals, [sens_S_on_μmax_rel, sens_S_on_Ks_rel, sens_S_on_Sin_rel], title="Normalized sensitivities", label=["S w.r.t. μmax" "S w.r.t. Ks" "S w.r.t. Sin"], xlabel="Time (hours)", linewidth=2)
...
# missing
plot(t_vals, [sens_X_on_μmax_rel, sens_X_on_Ks_rel, sens_X_on_Sin_rel], title="Normalized sensitivities", label=["X w.r.t. μmax" "X w.r.t. Ks" "X w.r.t. Sin"], xlabel="Time (hours)", linewidth=2)
...
# ╔═╡ 1ee0fbfb-0e64-4db5-bed9-caa2aff7cf0b
md"""
!!! note
...
# end
@model function monod_deviation()
	Ks_dev ~ Normal(0.015, 0.0030) # 20% standard deviation
	new_prob = remake(oprob, p = [:Ks => Ks_dev])
	return solve(new_prob, AutoTsit5(Rosenbrock23()), saveat=0.5);
end
...
begin
	dev_model = monod_deviation()
	chain = sample(dev_model, Prior(), 100)
	solutions = generated_quantities(dev_model, chain)
end;
...
begin
    p1 = plot(title="Monte Carlo of logistic growth", ylabel = "concentration (g/l)", xlabel = "time (hours)") # make an empty plot
    for solution in solutions
        plot!(solution, label = false, color =:gray, alpha = 0.4,
			 linestyle = :dot)
    end
	plot!(osol, label = "Original simulation", linewidth = 3)
    p1
end
...
# missing
x1 = [solution[:X][end] for solution in solutions];

# ╔═╡ 1b1d0997-b854-4445-8c19-e73db6784a9b
# missing
histogram(x1, xlims = [0.010, 0.015])
...
# ╔═╡ 6178c0ca-f8ab-49cf-aa9c-82c1dea9b8cf
md"""
!!! note
...
# end
begin
	t_star_idx = argmax(abs.(sens_X_on_Ks_rel))
	t_star = t_vals[t_star_idx]
...
#     S_meas ~ missing
# end
@model function monod_meas(t_meas)
    σ_X = 0.05 * 0.004   # ~5% relative noise
    σ_S = 0.05 * 0.011

    Ks_dev ~ truncated(Normal(0.015, 0.0030), lower = 0.0, upper = 1.0)

    sol = solve(remake(oprob, p = [:Ks => Ks_dev]), AutoTsit5(Rosenbrock23()), 	saveat=0.1)
    
    S_pred, X_pred = sol(t_meas)

    X_meas ~ Normal(X_pred, σ_X)
    S_meas ~ Normal(S_pred, σ_S)
	return solve(remake(oprob, p = [:Ks => Ks_dev]), saveat=0.5)
end

# ╔═╡ 11e105de-270c-434a-bfb4-9118dfc9e461
# monod_cond_model = missing
monod_cond_model = monod_meas(t_star) | (X_meas = X_meas, 
                                          S_meas = S_meas)
...
# monod_chain = missing
monod_chain = sample(monod_cond_model, NUTS(), 100)
...
# missing
solutions_monod = generated_quantities(monod_cond_model, monod_chain);
...
begin
    p = plot(ylabel = "concentration (g/l)", xlabel = "Time (hours)")
    for solution in solutions_monod
        plot!(solution, label=false, color=:grey, linestyle=:dot, 									alpha=0.4)
    end
	plot!(osol, label = ["Original simulation S" "Original simulation X"], linewidth = 3)
...
# missing
x2 = [solution_monod[:X][end] for solution_monod in solutions_monod];

# ╔═╡ 18059d0c-cf37-44a1-86a8-859ac66a66f0
# missing
histogram(x2, title="Posterior histogram of X", ylabel = "counts", xlabel="X", xlims = [0.010, 0.015])
