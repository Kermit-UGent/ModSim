...
# missing
@parameters k=0.2 Y=0.76 Q=2.0 V=40.0 Sin=2.2 n=0.5
...
# eqns_ferm = missing
eqns_ferm = [D(S) ~ (Q/V)*((Sin+n*B) - S) - k*X*S, D(X) ~ -(Q/V)*X + Y*k*S*X]
...
# missing
@mtkbuild sys_ferm = System(eqns_ferm, t)
...
# parms = missing
parms = [:k => 0.2, :Y => 0.76, :Q => 2, :V => 40, :Sin => 2.2, :n => 0.50]
...
# sprob_ferm = missing
sprob_ferm = SDEProblem(sys_ferm, u0, tspan, parms);
...
# ssol_ferm = missing
ssol_ferm = solve(sprob_ferm, EM(), dt=0.1)
...
# esprob_ferm = missing
esprob_ferm = EnsembleProblem(sprob_ferm)
...
# essol_ferm = missing
essol_ferm = solve(esprob_ferm, EM(), dt=0.1, trajectories=100)
...
# missing
plot(essol_ferm, ylim=(0.0,2.0), linealpha=0.5)
...
begin
	Xeq_values = []
	for i=1:length(essol_ferm)
		append!(Xeq_values, essol_ferm.u[i][:X][end])
	end
end
