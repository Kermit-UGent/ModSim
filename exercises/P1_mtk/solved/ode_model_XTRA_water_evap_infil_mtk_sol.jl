...
# @mtkbuild sys_wei = ODESystem(eqs_wei, t; continuous_events = [[W ~ 0]=>[O ~ 0]])
@mtkbuild sys1_wei = ODESystem(eqs_wei, t; continuous_events = [[W ~ 0]=>[O ~ 0]])
...
parms = [I=>2.7, O=>20.0, k₁=>0.4, k₂=>1.0]
...
oprob1_wei = ODEProblem(sys1_wei, u0, tspan, parms)
...
osol1_wei = solve(deepcopy(oprob1_wei), Tsit5(), saveat=0.1, reltol=1e-9)
...
# ╔═╡ 9e2caea2-bef6-4b28-92d4-05d06ba755ae
osol1_wei[end]
...
# ╔═╡ 290ca075-f570-4a12-8f84-a4164bd49bdd
@mtkbuild sys2_wei = ODESystem(eqs_wei, t; continuous_events = [[W ~ 0]=>[O ~ 0]], discrete_events = [[10.0]=>[O ~ 20], [15.0]=>[O ~ 20]])
...
oprob2_wei = ODEProblem(sys2_wei, u0, tspan, parms)
...
osol2_wei = solve(deepcopy(oprob2_wei), Tsit5(), saveat=0.1, reltol=1e-9)
...
eq_val = solve(SteadyStateProblem(sys1_wei, [W=>7.0, G=>7.0], [I=>2.7, O=>0.0, k₁=>0.4, k₂=>1.0]))
