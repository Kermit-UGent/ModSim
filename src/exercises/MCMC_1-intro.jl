...
# ╔═╡ 2ef471da-3edb-44cc-b217-87c124d6d0b5
α = rand(Exponential(10))

# ╔═╡ af79b79d-9926-4c20-ae1a-8d38bd7f1a88
α*times[1]

# ╔═╡ c1304055-b99a-4570-95cf-72724e64c1db
plot(Exponential(10))

# ╔═╡ dc38d9fb-2cc4-40d1-bcd7-956ec75f6b8f
rand(Poisson(α*times[1]))

# ╔═╡ 85d75659-5294-46e8-ad85-dac357af072c
plot(Poisson(α*times[1]))

# ╔═╡ e9d2f2ca-48ed-4cd8-bd0a-ff0ef640c8fb
let
	@model function mutations(ts)
		# Exponential(λ) -> λ is the average rate of mutations
		α ~ Exponential(10)
		# α ~ Exponential()    # also works
		# α ~ LogNormal()      # also works
		# α ~ Uniform(0, 100)  # also works
		
		num_mutations = zeros(length(ts))
		for i in eachindex(ts)
			# Poisson(α * ts[i]) -> the probability of having α*ts[i] mutations
...
# Hence, a few mutations (bp = base pair) per My, is here 10 mutations per My on average.
...
histogram(chain[:α], normalized=:probability)
...
mutation_chain = sample(conditioned_model, NUTS(), 2000)
# mutation_chain = sample(conditioned_model, MH(), 2000)

# ╔═╡ dfc7a6d6-715d-4b3f-ae58-7b930616c41c
mutation_chain

# ╔═╡ 11199425-5e4f-4fc5-b984-5522321b3378
# Gives a list of anonymous functions. The plot instruction will
# fill in the time t automatically.
[t -> αᵢ*t for αᵢ in mutation_chain[:α][1:10:end]]
...
begin
	...
	# end
	# plot!()
	plot!([t -> αᵢ*t for αᵢ in mutation_chain[:α][1:10:end]], color = :purple, opacity = 0.05, label = false)
end
...
# ╔═╡ d9b6e99d-e483-4633-88eb-5a7824716eb8
std(sp_alpha)
...
@model function horsetations(ts)
	...
	return (α, num_mutations, fossil_age, horse_mutations)
end
...
# ╔═╡ 37856d0c-a378-47c0-b29a-967fd12982de
horseditioned_model()
...
# ╔═╡ 960c95f8-4431-4e56-97fc-783bef811891
mean(horse_chain[:fossil_age])

# ╔═╡ d308bff0-3ed5-4cb1-b81c-f53f7a1967a1
std(horse_chain[:fossil_age])
