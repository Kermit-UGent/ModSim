...
md"""
Check the order of the species (state variables):
"""
...
md"""
Solve the ODE problem using `Tsit5()`, `saveat=0.5` and the callback function `cb`. Store the solution in `osol`:
"""
...
2. If W drops then also G (with some delay) because there is an exchange of water between W and G. Beside W and G need to be in equilibrium.
