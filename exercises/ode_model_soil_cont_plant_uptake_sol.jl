...
md"""
Suppose that we start an experiment in which we simulate the evoluation of the pollutant in the soil and plant during $400$ days. The inital pollutant concentrations in the soil and plant both have the value of $0.001\;mg/kg$. In the experiment, the soil is being contaminated at a rate $0.06\;mg/(kg \cdot day)$. The degradation rates and uptake coefficient have the following values: $k_1 = 4.1 \times 10^{-3}\;day^{-1}$, $k_2 = 1.9 \times 10^{-2}\;kg/(mg \cdot day)$, $k_3 = 2.2 \times 10^{-2}\;day^{-1}$.
"""
