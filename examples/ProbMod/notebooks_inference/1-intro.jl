...
md"""
According to the [molecular clock hypothesis](https://en.wikipedia.org/wiki/Molecular_clock), the mutation rate of nucleotide and protein sequences is roughly linear over time, and the same for all species. While this is a bit of an oversimplification, the concept has become an important tool in evolutionary biology to estimate how long ago species have diverged.
"""
...
# ╔═╡ ba60c447-2b16-46a7-ac84-998b392fd710
@model function _mutations(ts)
	α ~ Exponential(10)
...
# ╔═╡ 8551c81d-1feb-4eb1-9cdf-22f67cb3b423
_mutation_model = _mutations(times);
...
md"""
What we're after is our updated belief on the distribution of `α` given the observed data. We can do this by using the `sample` function on our model. We no longer use `Prior()` as second input, and instead choose one of the following sampling algorithm, explained in the theory lectures:
- `MH()`: Metropolis-Hastings sampler
- `Gibbs()`: Gibbs sampler
- `HMC()`: Hamiltonian Monte Carlo sampler
- `NUTS()`: No-U-Turn sampler

You can find more information about them in the corresponding Julia docs. In practice, `NUTS()` is often an excellent choice if all variables are continuous.
"""
