...
md"""
In practical 1, we learned to solve ODEs by making use of ModelingToolkit.jl (MTK). In this practicum we will learn to use Catalyst.jl which is an alternative for modeling and simulating ODEs. So bear in mind that all problems you will solve in this practicum can also be solved
...
parms1 = [:α => 0.08, :β => 1.0e-6, :r => r, :m => 0.4]
