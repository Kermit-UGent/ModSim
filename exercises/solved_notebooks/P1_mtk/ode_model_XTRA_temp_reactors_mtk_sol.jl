...
# https://users.ugent.be/~gvhaelew/fig/temperature_reactors.png
md"""
...
# @parameters 
@parameters Q=0.05e-3 V₁=0.004 V₂=0.006 Tin=40 λ=240 A₁=0.1 A₂=0.2 cₚ=4200 ρ=1.0e3 Pₕ=5000 Tₐ=20
...
# eq_T1 = missing
eq_T1 = cₚ*ρ*V₁*D(T₁) ~ cₚ*ρ*Q*(Tin - T₁) - λ*A₁*(T₁ - Tₐ)
# eq_T1 = D(T₁) ~ Q/V₁*(Tin - T₁) - λ*A₁/(cₚ*ρ*V₁)*(T₁ - Tₐ)  # alternative
...
# eq_T2 = missing
eq_T2 = cₚ*ρ*V₂*D(T₂) ~ cₚ*ρ*Q*(T₁ - T₂) - λ*A₂*(T₂ - Tₐ) + Pₕ
# eq_T2 = D(T₂) ~ Q/V₂*(T₁ - T₂) - λ*A₂/(cₚ*ρ*V₂)*(T₂ - Tₐ) + Pₕ/(cₚ*ρ*V₂)
...
# @mtkbuild missing
@mtkbuild sys1_reactors = ODESystem(eqns_reactors, t)
...
oprob1_reactors = ODEProblem(sys1_reactors, [T₁=>15.0, T₂=>15.0], (0.0, 600.0), [])
...
sol1_reactors = solve(oprob1_reactors, Tsit5(), saveat=1)
...
# ╔═╡ 177a9e17-c679-4203-ac90-dea002e54072
# ╠═╡ disabled = true
#=╠═╡
sol1_reactors[end]
  ╠═╡ =#

# ╔═╡ 0f9ac188-7609-4982-96d3-9c5ede248e3d
# ╠═╡ disabled = true
#=╠═╡
sol1_reactors[T₁]
  ╠═╡ =#
...
# ╔═╡ fb6dcf86-619e-4d7f-be24-14950a8c6718
# ╠═╡ disabled = true
#=╠═╡
sol1_reactors[T₂]
  ╠═╡ =#
...
equil_T_vals = solve(SteadyStateProblem(sys1_reactors, [T₁=>37, T₂=>53], []))
...
"""
#=
1. Because some heat is dissipated through the walls of reactor 1.
2. Because heat is added to reactor 2 through Ph.
=#
...
# @mtkbuild missing
@mtkbuild sys2_reactors = ODESystem(eqns_reactors, t; discrete_events=[[600]=>[Tin ~ 45.0], [600]=>[Pₕ ~ 2250]])
# @mtkbuild sys2_reactors = ODESystem(eqns_reactors, t; discrete_events=[[600]=>[Tin ~ 45.0]])
# @mtkbuild sys2_reactors = ODESystem(eqns_reactors, t; discrete_events=[[600]=>[Pₕ ~ 2250]])
...
oprob2_reactors = ODEProblem(sys2_reactors, [T₁=>15.0, T₂=>15.0], (0.0, 1200.0), [])
...
sol2_reactors = solve(deepcopy(oprob2_reactors), Tsit5(), saveat=1)
...
# ╔═╡ 3c26e5e9-bb6a-4022-9cfd-04aad2fcf6cc
sol2_reactors[end]
...
"""
#=
1. T1 is only affected by Tin, directly.
2. T2 is affected by both Tin and Ph. T2 is indirectly affected by Tin (through T1). T2 is directly affected by Ph.
=#
