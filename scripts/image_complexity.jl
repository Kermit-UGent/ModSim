...
using Colors, Images, Plots, PlutoUI, ColorSchemes, ImageIO
...
# ╔═╡ 3e925c2a-9b15-45c9-9122-36f7632de56c
cs = cgrad(:gist_ncar)

# ╔═╡ c54910df-405c-4d00-8cd7-e1dc1015e30c
function compute_mandelbrot(xmin=-2.0, xmax=0.5, ymin=-1.25, ymax=1.25;
			width=n, height=m, maxiter=200, colorscheme=cs)
    x = range(xmin, stop=xmax, length=width)
    y = range(ymin, stop=ymax, length=height)

    #img = [RGB(0, 0, mandelbrot(xi + yj*im, maxiter)/maxiter) for xi in x, yj in y]
	img = [get(cs, mandelbrot(xi + yj*im, maxiter)/maxiter) for xi in x, yj in y]
    return img
end

# ╔═╡ 98f8027f-b2d5-46eb-866b-ba3b1296e3a7
get(cs, 0.7)
...
# ╠═╡ disabled = true
#=╠═╡
imgmandelbrot_huge = compute_mandelbrot(width=5000, height=5000)
  ╠═╡ =#

# ╔═╡ ea874791-ea70-4bc3-8db6-a3a6aea980b9
#=╠═╡
save("mandlebrot_huge.png", imgmandelbrot_huge)
  ╠═╡ =#

# ╔═╡ 009bbf67-ed83-4be9-9e86-c989ca804a3a
imgmandelbrot2 = compute_mandelbrot(-0.75-0.01, -0.75+0.01, + 0.11 - 0.01, 0.11+0.01)
...
# ╔═╡ 4221299c-ad68-4aeb-8aa8-34ab8c8b47be
begin
...
# ╔═╡ a277a2c1-2f54-4dd3-9b3a-f14ef682164a
...
manifest_format = "2.0"
project_hash = "a7455687e7c75885e4eebf544c917582edbf6d19"
